    ...
    {modules.map((module, moduleIndex) => <section key={module.id} style={{ ...box, padding: 0, overflow: "hidden" }}>
      ...
      <div style={{ display: "grid" }}>
        {(module.items || []).map((item, itemIndex) => {
          ...
          if (item.item_type === "resource") return <a key={item.id} href={`${API_BASE}${item.resource_path}`} download={item.resource_name || true} style={{ padding: "16px 20px", borderTop, color: "#1d4ed8", fontWeight: 800, textDecoration: "none" }}>📎 Download: {item.title || item.resource_name}</a>
