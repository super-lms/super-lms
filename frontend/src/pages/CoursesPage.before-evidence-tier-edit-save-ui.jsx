...
export default function CoursesPage() {
  ...
  async function saveLearningCategory(courseId, categoryId) {
    ...
    if (!name) {
      setError("Learning category name is required.")
      setMessage("")
      return
    }

    if (!Number.isFinite(weight) || weight <= 0) {
      setError("Learning category weight must be greater than 0.")
      setMessage("")
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Failed to update learning category")
      }

      setMessage(`Learning category updated: ${data.name || name}`)
      cancelEditLearningCategory()

      setActiveCompetencyCourseId(null)
      await loadCompetencies(courseId)
    } catch (err) {
      setError(err.message || "Failed to update learning category")
    } finally {
  ...
  async function deleteLearningCategory(courseId, category) {
    const categoryName = String(category?.name || "this learning category")

    const confirmed = window.confirm(
      `Delete learning category "${categoryName}"? This is only allowed when no evidence tiers remain.`
    )
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Failed to delete learning category")
      }

      setMessage(`Learning category deleted: ${data.deleted?.name || categoryName}`)

      setActiveCompetencyCourseId(null)
      await loadCompetencies(courseId)
    } catch (err) {
      setError(err.message || "Failed to delete learning category")
    } finally {
  ...
  async function createLearningCategory(courseId) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Failed to create learning category")
      }
      ...
      setMessage(`Learning category created: ${data.name || name}`)

      setActiveCompetencyCourseId(null)
      await loadCompetencies(courseId)
    } catch (err) {
      setError(err.message || "Failed to create learning category")
    } finally {
  ...
  async function loadCompetencies(courseId) {
    ...
    try {
      ...
      if (!categoriesRes.ok) {
        throw new Error(categoriesData.error || "Failed to load learning categories")
      }
                ...
                <div key={course.id} style={courseCardStyle}>
                  ...
                  {isCompetenciesOpen ? (
                    <div style={learningPathBoxStyle}>
                      <h3 style={{ marginTop: 0, marginBottom: "8px" }}>Learning Categories</h3>

                      <p style={{ color: "#4b5563", lineHeight: 1.5 }}>
                        These are the competency-based learning categories and evidence tiers for this course.
                      ...
                      <div style={{ border: "1px solid #d7dce5", borderRadius: "12px", padding: "14px", marginBottom: "14px", background: "#ffffff" }}>
                        <h4 style={{ marginTop: 0, marginBottom: "10px" }}>Add Learning Category</h4>
                        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 160px", gap: "10px" }}>
                          <div>
                            <label style={labelStyle}>Learning Category Name</label>
                            <input
                        ...
                        <div style={{ marginTop: "12px" }}>
                          <button
                            ...
                            style={primaryButtonStyle}
                          >
                            {creatingLearningCategoryCourseId === course.id ? "Adding..." : "Add Learning Category"}
                          </button>
                        </div>
                      </div>

                      {courseCompetencies.length === 0 ? (
                        <div style={emptyLearningPathStyle}>
                          <strong>No Learning Categories Yet</strong>
                          <div style={{ marginTop: "6px", color: "#4b5563", lineHeight: 1.5 }}>
                            This custom competency course is ready for teacher-created learning categories.
                          </div>
                        </div>
                      ) : (
                                  ...
                                  {editingLearningCategoryId === category.id ? (
                                    ...
                                    <>
                                      ...
                                      <div style={{ color: "#4b5563" }}>
                                        Learning Category Weight: {category.weight_percent || "0.00"}%
                                      </div>
                                    </>
                                  )}
                                </div>
                                      ...
                                      <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
                                        ...
                                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                                          <button
                                            type="button"
                                            onClick={() => {
                                              setMessage("Evidence tier editing UI is coming next.")
                                              setError("")
                                            }}
                                            style={buttonStyle}
                                          >
