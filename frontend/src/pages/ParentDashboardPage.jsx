...
export default function ParentDashboardPage() {
  ...
  const [parentPortalLanguage, setParentPortalLanguage] = useState("en")
  ...
  const parentCoachRecommendation = useMemo(() => {
    if (errorText) {
      return {
        title: "Ask the school for help",
        reason:
          "Something is preventing the parent dashboard from loading correctly.",
        action:
          "Contact the school or teacher and share the message shown on this page.",
      }
    }

    if (!children || children.length === 0) {
      return {
        title: "Check child access",
        reason:
          "No linked child account is showing in this parent portal.",
        action:
          "Ask the school to confirm your parent email is connected to your child’s student account.",
      }
    }
    ...
    if (!selectedChildEmail) {
      return {
        ...
        reason:
          "Parent guidance depends on which child you are reviewing.",
        action:
          "Select your child to view courses, assignments, feedback, and progress.",
      }
    }
    ...
    if (!selectedCourseId) {
      return {
        ...
        reason:
          "Course progress, missing work, and feedback are easier to understand one course at a time.",
        action:
          "Select a course, then review the learning snapshot and assignment list.",
      }
    }

    if (visibleAssignments.length === 0) {
      return {
        title: "Watch for posted assignments",
        reason:
          "No assignments are currently showing for this child in the selected course.",
        action:
          "Check back later, or ask your child whether the teacher has posted work yet.",
      }
    }

    if (progressPercent < 70) {
      return {
        title: "Review missing or unfinished work",
        reason:
          "The current completion rate suggests there may be work to discuss at home.",
        action:
          "Look at assignments not yet submitted, then ask your child what support or time they need.",
      }
    }

    return {
      title: "Review feedback with your child",
      reason:
        "Assignments and progress are visible. The best next step is a learning conversation.",
      action:
        "Look at teacher feedback, celebrate what is going well, and choose one next step together.",
    }
  }, [errorText, children, selectedChildEmail, selectedCourseId, visibleAssignments, progressPercent])
  ...
  return (
    <>
          ...
          <SectionHeader
            ...
            action={
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                <ActionButton
                  quiet={parentPortalLanguage !== "en"}
                  onClick={() => setParentPortalLanguage("en")}
                >
                  English
                </ActionButton>

                <ActionButton
                  quiet={parentPortalLanguage !== "zh"}
                  onClick={() => setParentPortalLanguage("zh")}
                >
                  Chinese
                </ActionButton>
      ...
      <FloatingTeacherCoach
        title="Parent Coach"
        subtitle="Parent Support Coach"
        recommendationTitle={parentCoachRecommendation.title}
        recommendationReason={parentCoachRecommendation.reason}
        recommendationAction={parentCoachRecommendation.action}
      >
        <div style={{ fontWeight: 900, marginBottom: "6px" }}>Parent workflow</div>
        <div style={{ color: "#111827", lineHeight: 1.55 }}>
          <div>□ Choose your child</div>
          <div>□ Choose a course</div>
          <div>□ Review progress and feedback</div>
          <div>□ Check for missing or unfinished work</div>
          <div>□ Ask one supportive question at home</div>
        </div>
      </FloatingTeacherCoach>
    </>
  )
