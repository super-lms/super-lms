...
export default function ClassEnrollmentPage() {
  ...
  const loadCourses = async () => {
    ...
    try {
      ...
      const carrieCourses = safeCourses.filter((course) => {
        const teacherEmailValue = String(course?.teacher_email || "")
          .trim()
          .toLowerCase()

        return teacherEmailValue === "carriefang@cbcschools.ca"
      })

      setCourses(carrieCourses)

      if (carrieCourses.length === 0) {
        setCoursesMessage("No assigned courses found.")
        return
      }

      setCoursesMessage(`${carrieCourses.length} assigned course${carrieCourses.length === 1 ? "" : "s"} loaded.`)

      const firstCourse = carrieCourses[0]
    ...
    <div>
      ...
      <div style={sectionStyle}>
        ...
        <p style={helperTextStyle}>
          Pick one of Carrie Fang&apos;s assigned courses so the class name stays consistent across import and manual entry.
        </p>
