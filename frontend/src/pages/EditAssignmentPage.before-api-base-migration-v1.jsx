...
export default function EditAssignmentPage() {
  ...
  useEffect(() => {
    if (window.location.hash !== "#exam-sections") return;

    const firstTimer = window.setTimeout(() => {
      const target = document.getElementById("exam-sections");
  ...
  function duplicateAssignmentSection(index) {
    setAssignmentSections((currentSections) => {
      ...
      const duplicatedSection = {
        section_name: `${String(sectionToDuplicate.section_name || "Untitled Section").trim()} Copy`,
        competency_bucket: sectionToDuplicate.competency_bucket || "KNOW",
        max_points: sectionToDuplicate.max_points || 10,
        section_weight: sectionToDuplicate.section_weight || 1,
      };

      return [
        ...currentSections.slice(0, index + 1),
        duplicatedSection,
        ...currentSections.slice(index + 1),
      ];
    });
  ...
  function moveAssignmentSection(index, direction) {
    setAssignmentSections((currentSections) => {
      ...
      const sectionToMove = reorderedSections[index];

      reorderedSections[index] = reorderedSections[nextIndex];
      reorderedSections[nextIndex] = sectionToMove;
  ...
  const assignmentCoachRecommendation = useMemo(() => {
    const cleanTitle = String(title || "").trim();
    const hasRubric = Boolean(rubric) && rubricCriteria.length > 0;
    const savedSections = assignmentSections.filter((section) => {
      return Boolean(section?.id);
    });
    const hasSavedSections = savedSections.length > 0;
    const sectionBuckets = assignmentSections.map((section) =>
      String(section?.competency_bucket || "").trim().toUpperCase()
    );
    const hasKnow = sectionBuckets.includes("KNOW");
    const hasDo = sectionBuckets.includes("DO");
    const hasUnderstand = sectionBuckets.includes("UNDERSTAND");

    if (!cleanTitle) {
      return {
        title: "Add the assignment title",
        reason:
          "The assignment needs a clear title before it is ready for students, parents, grading, or reports.",
        action:
          "Complete Step 1 first. Add the assignment title, description, and due date before continuing.",
        buttonLabel: "",
        buttonAction: null,
      };
    }
    ...
    if (!selectedCategory || !selectedSubcategory) {
      return {
        title: "Choose the assessment group",
        reason:
          "SUPER LMS needs to know where this assignment belongs so the KDU score can flow into the correct course pathway and evidence tier.",
        action:
          "Go to Step 2 and choose the assessment group and evidence tier for this assignment.",
        buttonLabel: "",
        buttonAction: null,
      };
    }
    ...
    if (!hasRubric) {
      return {
        title: "Build the KDU rubric",
        reason:
          "The grading screen works best when KNOW, DO, and UNDERSTAND criteria are already attached to the assignment.",
        action:
          "Go to Step 3 and use Build KDU Rubric. Then review and save the criteria.",
        buttonLabel: "",
        buttonAction: null,
      };
    }
    ...
    if (!hasSavedSections) {
      return {
        title: "Build and save exam sections",
        reason:
          "Raw mark entry needs saved sections so SUPER LMS knows how to convert marks into KNOW, DO, and UNDERSTAND evidence.",
        action:
          "Go to Step 4. Add section names, KDU buckets, out-of marks, and section weights. Then click Save Exam Sections.",
        buttonLabel: "Take Me There",
        buttonAction: () => scrollToElementById("exam-sections"),
      };
    }

    if (!hasKnow || !hasDo || !hasUnderstand) {
      return {
        title: "Check KDU coverage",
        reason:
          "This assignment has saved sections, but it may not yet include all three KDU evidence types.",
        action:
          "Go to Step 4 and confirm whether this assignment should include KNOW, DO, and UNDERSTAND sections before grading.",
        buttonLabel: "Check Sections",
        buttonAction: () => scrollToElementById("exam-sections"),
      };
    }

    return {
      title: "Open grading when ready",
      reason:
        "The assignment has a pathway, rubric, and saved raw-mark sections. It appears ready for student mark entry.",
      action:
        "Review the setup one final time, then open the grading page to enter marks or feedback.",
      buttonLabel: "Open Grade Page",
      buttonAction: openGradePage,
    };
  }, [
    title,
    rubric,
    ...
    <>
      ...
      <section className="panel">
        ...
        <div style={{ display: "grid", gap: "18px", maxWidth: "1120px" }}>
          ...
          <div id="exam-sections"><WorkflowStep number="4" title="Exam Section Builder">
            <div style={{ display: "grid", gap: "14px" }}>
                        ...
                        <td style={tableCellStyle}>
                          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                            <EditAssignmentActionButton
                              ...
                              disabled={sectionSaving || index === 0}
                            >
                              Move Up
                            </EditAssignmentActionButton>
                            ...
                            <EditAssignmentActionButton
                              ...
                              disabled={
                                sectionSaving ||
                                index === assignmentSections.length - 1
                              }
                            >
                              Move Down
                            </EditAssignmentActionButton>
                            ...
                            <EditAssignmentActionButton
                              quiet
                              onClick={() => duplicateAssignmentSection(index)}
      ...
      <FloatingTeacherCoach
        ...
        recommendationTitle={assignmentCoachRecommendation.title}
        recommendationReason={assignmentCoachRecommendation.reason}
        recommendationAction={assignmentCoachRecommendation.action}
        actionButtonLabel={assignmentCoachRecommendation.buttonLabel}
        onAction={assignmentCoachRecommendation.buttonAction}
      >
        <div style={{ fontWeight: 900, marginBottom: "6px" }}>Teacher workflow</div>
        <div style={{ color: "#111827", lineHeight: 1.55 }}>
          <div>□ Step 1: Assignment details are clear</div>
          <div>□ Step 2: Assessment group is selected</div>
          <div>□ Step 3: KDU rubric is ready</div>
          <div>□ Step 4: Exam sections are saved</div>
          <div>□ Step 5: Open grading when ready</div>
        </div>
      </FloatingTeacherCoach>
    </>
  );
