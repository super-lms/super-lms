...
function ReportsPage() {
  ...
  useEffect(() => {
    fetch("/api/enrolled-students")
      .then((res) => {
        if (!res.ok) {
          throw new Error("Could not load enrolled students");
        }
        return res.json();
      })
      .then((data) => setEnrolledStudents(Array.isArray(data) ? data : []))
      .catch(() => setMessage("Could not load enrolled students"));
  }, []);
  ...
  function saveComment(studentUserId, com1, com2) {
    ...
    .then(res => res.json())
    .then(() => setMessage("Comment saved"))
    .catch(() => setMessage("Failed to save comment"));
  }
            ...
            <tbody>
              {reportComments.map((student) => (
                <tr key={student.student_user_id}>
                  ...
                  <td style={tdStyle}>
                    <textarea
                      defaultValue={student.com1}
                      onChange={(e) => student.com1 = e.target.value}
                  ...
                  <td style={tdStyle}>
                    <textarea
                      defaultValue={student.com2}
                      onChange={(e) => student.com2 = e.target.value}
                      style={{ width: "100%", minHeight: "60px" }}
                    />
                  </td>
...
{reportData && (
        <div style={sectionStyle}>
          <h2 style={sectionTitleStyle}>{reportData.course_title}</h2>
