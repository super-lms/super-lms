...
export default function AssignmentsPage() {
  ...
  function loadStudentAssignments() {
    ...
    return fetch(`http://localhost:3000/api/students/${encodeURIComponent(user.email)}/assignments`)
      .then((res) => {
  ...
  function loadStudentSubmissions() {
    ...
    return fetch(`http://localhost:3000/api/students/${encodeURIComponent(user.email)}/submissions`)
      .then((res) => {
  ...
  function loadClassStudents(classId) {
    ...
    return fetch(`http://localhost:3000/api/class-roster/${classId}`)
      .then((res) => {
  ...
  function loadCategoriesForClass(classId) {
    ...
    return fetch(`http://localhost:3000/api/courses/${classId}/categories`)
      .then((res) => {
      ...
      .then((data) => {
        ...
        return Promise.all(
          nextCategories.map((category) =>
            fetch(`http://localhost:3000/api/categories/${category.id}/subcategories`)
              .then((res) => {
  ...
  function createAssignment(e) {
    ...
    if (!selectedSubcategoryId) {
      setError("Select a subcategory for this assignment")
      return
    }
  ...
  function importAssignmentsFromCsv() {
    ...
    fetch("http://localhost:3000/api/assignments/import", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        class_id: Number(selectedClassId),
  ...
  function confirmDeleteAssignment() {
    ...
    fetch(`http://localhost:3000/api/assignments/${deleteTargetAssignment.id}`, {
      method: "DELETE",
    })
      .then(async (res) => {
        const data = await res.json().catch(() => ({}))
  ...
  function addStudentToRoster(e) {
    ...
    fetch(`http://localhost:3000/api/classes/${selectedClassId}/students`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
            ...
            >
              ...
              >
                <div style={{ display: "grid", gap: "10px", fontSize: "0.95rem", lineHeight: 1.5 }}>
                  ...
                  <div>
                    <strong>Category:</strong> {deleteTargetAssignment.category_name || "Not linked"}
                  </div>
                  <div>
                    <strong>Subcategory:</strong> {deleteTargetAssignment.subcategory_name || "Not linked"}
                  </div>
              ...
              {selectedClassId && teacherSection === "create" ? (
                <div ref={createSectionRef}>
                  <DetailCard title="Create Assignment">
                    {!classHasCategories ? (
                      <NoticeBox>
                        This class does not have grading categories available yet. Load or create categories before creating assignments linked to grading structure.
                      </NoticeBox>
                    ) : null}
                        ...
                        <div>
                          <DetailCard title="Assignment Weight Preview">
                            <div style={{ display: "grid", gap: "12px" }}>
                              ...
                              <div>
                                <strong>Selected subcategory:</strong>{" "}
                              ...
                              <div>
                                ...
                                {previewCalculatedWeight === null ? "Select category and subcategory" : formatPercent(previewCalculatedWeight)}
                              </div>
                            </div>
                          </DetailCard>
                        </div>
                          ...
                          return (
                            <DetailCard key={assignment.id} title={assignment.title}>
                              <div style={{ display: "grid", gap: "8px" }}>
                                ...
                                <div>
                                  <strong>Category:</strong> {assignment.category_name || "Not linked"}
                                </div>
                                <div>
                                  <strong>Subcategory:</strong> {assignment.subcategory_name || "Not linked"}
                                </div>
                                <div>
