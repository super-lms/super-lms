...
export default function GradebookPage() {
  ...
  async function saveKduScore(student, assignment, match, overrideDraft = null) {
    ...
    setStatus(`Auto-saving KDU score for ${selectedStudent.student_name}...`);
    ...
    try {
      ...
      setStatus(`Saved KDU score for ${selectedStudent.student_name}.`);
      setCellSaveStatus((current) => ({
        ...current,
        [key]: "Saved ✓",
      }));
  ...
  const classSummary = useMemo(() => {
    const gradedStudents = students.filter((student) =>
      Number.isFinite(Number(selectedStudent.current_percent))
    );
      ...
      classAverage:
        gradedStudents.length === 0
          ...
          : gradedStudents.reduce(
              (sum, student) => sum + Number(selectedStudent.current_percent),
              0
            ) / gradedStudents.length,
      extending: students.filter(
        (student) => getProficiency(selectedStudent.current_percent) === "Extending"
      ).length,
      proficient: students.filter(
        (student) => getProficiency(selectedStudent.current_percent) === "Proficient"
      ).length,
      developing: students.filter(
        (student) => getProficiency(selectedStudent.current_percent) === "Developing"
      ).length,
      emerging: students.filter(
        (student) => getProficiency(selectedStudent.current_percent) === "Emerging"
      ).length,
      notGraded: students.filter(
        (student) => getProficiency(selectedStudent.current_percent) === "Not graded"
      ).length,
              ...
              <table>
                ...
                <tbody>
                  {students.map((student) => {
                    ...
                    return (
                      <tr
                        key={selectedStudent.student_user_id}
                        id={
                          studentMatchesFocus
                      ...
                      >
                        ...
                        <td>
                          <strong>{formatPercent(selectedStudent.current_percent)}</strong>
                        </td>
                        <td>{getProficiency(selectedStudent.current_percent)}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <section className="panel">
            ...
            {selectedStudent ? (
              <div style={{ display: "grid", gap: "18px" }}>
                <div
                  key={`competency-snapshot-${selectedStudent.student_user_id}`}
                  style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
                    <div>
                      ...
                      <div style={{ color: "#4b5563" }}>
                        Current Course Standing: <strong>{formatPercent(selectedStudent.current_percent)}</strong>
                    ...
                    <button
                      type="button"
                      className="secondary-button"
                      onClick={() => toggleCompetencySnapshot(selectedStudent.student_user_id)}
                    >
                      {expandedCompetencyStudents[selectedStudent.student_user_id] ? "Hide Snapshot" : "Show Snapshot"}
                    </button>
                  </div>

                  {expandedCompetencyStudents[selectedStudent.student_user_id] ? (
                    !selectedStudent.group_breakdown || selectedStudent.group_breakdown.length === 0 ? (
                    <div style={{ color: "#4b5563" }}>
                    ...
                    <div className="table-wrap">
                      <table>
                        ...
                        <tbody>
                          {selectedStudent.group_breakdown.map((group) => (
                            <tr key={`${selectedStudent.student_user_id}-${group.subcategory_id}`}>
              ...
              <div style={{ color: "#4b5563" }}>
                Select a student to view competency details.
              </div>
            )}
          </section>
