...
function CourseOverviewCard({ course, isSelected, onSelect }) {
  return (
    ...
    >
      <div style={{ fontWeight: 800, marginBottom: "6px" }}>{course.title || "Untitled Course"}</div>
...
function ResultCard({ assignment, submissionState, onOpen }) {
  ...
  const submissionStatusLabel = formatSubmissionStatus(
  ...
  return (
    ...
    >
      <SectionHeader
        ...
        subtitle="Write or update your submission here."
      ...
      {submissionLoading ? (
        ...
        <>
          ...
          <div style={{ marginBottom: "16px" }}>
            <label style={labelStyle}>Submission Text</label>
            <textarea
              rows="10"
              value={draftText}
...
export default function StudentDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadStudentDashboard() {
      ...
      try {
        const [coursesResponse, lessonsResponse, assignmentsResponse, enrolledStudentsResponse] =
          await Promise.all([
        ...
        const [coursesData, lessonsData, assignmentsData, enrolledStudentsData] =
          await Promise.all([
  ...
  const filteredLessons = useMemo(() => {
    ...
    return lessons.filter((lesson) => {
      return String(getLessonCourseId(lesson)) === String(selectedCourseId)
    })
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return assignments.filter((assignment) => {
      return String(getAssignmentCourseId(assignment)) === String(selectedCourseId)
    })
  ...
  const selectedSubmissionAssignment = useMemo(() => {
    ...
    return (
      assignments.find(
        (assignment) => String(assignment.id) === String(selectedSubmissionAssignmentId)
      ) || null
    )
  ...
  useEffect(() => {
    ...
    async function loadVisibleSubmissionStatuses() {
      ...
      if (!Array.isArray(visibleAssignments) || visibleAssignments.length === 0) {
        return
      }
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      submissionEditorRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "start",
      })
    }, 0)
  ...
  async function handleSaveSubmission() {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/assignments/${assignmentId}/student-submit`,
        {
          method: "POST",
          headers: {
      ...
      <div className="content-area">
        ...
        >
          <SummaryCard
            ...
            helper="Courses currently visible in the student portal."
          />
          <SummaryCard
            label="Due Soon"
            value={dueSoonCount}
            helper={
              selectedCourse
                ? `Assignments approaching due date in ${selectedCourse.title}.`
                : "Assignments approaching due date across the portal."
            }
          />
          <SummaryCard
            label="Submitted"
            ...
            helper={
              selectedCourseId
                ? "Assignments already submitted in the selected course."
                : "Assignments already submitted across the portal."
            }
          />
          <SummaryCard
            label="Graded"
            ...
            helper={
              selectedCourseId
                ? "Assignments with returned scores in the selected course."
                : "Assignments with returned scores across the portal."
            }
          />
          <SummaryCard
            label="Average"
            value={formatAverage(gradedAverage)}
            helper="Average of graded assignment scores currently shown."
          />
          <SummaryCard
            label="Lessons"
            ...
            helper={
              selectedCourseId
                ...
                : "Total lessons loaded into the student view."
            }
          />
          <SummaryCard
            label="Assignments"
            ...
            helper={
              selectedCourseId
                ...
                : "Total assignments loaded into the student view."
            }
          />
        </section>
        ...
        <section className="panel">
          <SectionHeader
            title="My Courses"
            subtitle="Choose a course to focus the dashboard on that class."
          />
          ...
          {selectedCourse ? (
            <p style={{ marginTop: "16px", marginBottom: 0, color: "#4b5563" }}>
              Viewing learning materials for <strong>{selectedCourse.title}</strong>.
            </p>
          ) : (
            <p style={{ marginTop: "16px", marginBottom: 0, color: "#4b5563" }}>
        ...
        >
          <div className="panel">
            <SectionHeader
              ...
              subtitle={
                selectedCourse
                  ? `Assignments scheduled in ${selectedCourse.title}.`
                  : "Assignments scheduled across your visible courses."
              }
            />
          ...
          <div className="panel">
            <SectionHeader
              ...
              subtitle="A quick read of your current dashboard view."
            />

            <div style={{ display: "grid", gap: "12px" }}>
              <DetailCard title="Selected Course">
                <div>{selectedCourse?.title || "No course selected"}</div>
              </DetailCard>
        ...
        <section className="panel">
          <SectionHeader
            title="My Results"
            subtitle="See returned scores and teacher feedback for your visible assignments."
          />
        ...
        <section className="panel">
          <SectionHeader
            ...
            subtitle="Lessons available for the currently selected course."
          />
        ...
        <section className="panel">
          <SectionHeader
            title="All Course Assignments"
            subtitle="A complete list of assignments for the currently selected course."
          />
