...
const SEMESTER_1_TEACHERS = [
  ...
  },
  {
    id: 9,
    name: "MYSTERY X",
    courses: ["Chem 11A", "Chem 11B", "Chem11C", "Chem 12A", "Chem 12B", "Chem 12C"],
    assignments: {
...
const SEMESTER_2_TEACHERS = SEMESTER_1_TEACHERS.map((teacher) => ({
  ...teacher,
  assignments: {
...
function getSubjectFromCourse(course) {
  ...
  if (value.startsWith("Chem 12")) return "Chem 12"
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={rulesGridStyle}>
        <RuleCard title="Full Year View" text="Semester 1 and Semester 2 appear on the same page." />
        <RuleCard title="Edit Courses" text="Change a course name in the All Courses column and scheduled blocks update automatically." />
        <RuleCard title="Add Courses" text="Add a new course to a teacher. It starts as unscheduled and can be placed into a block." />
...
function TimetableSemesterSection({ title, semesterKey, teachers, updateSemesterTeachers }) {
  ...
  const [newCourseDrafts, setNewCourseDrafts] = useState({})
  const summary = useMemo(() => buildSummary(teachers), [teachers])

  function addTeacher() {
    const newTeacherName = window.prompt("New teacher name:")

    if (newTeacherName === null) return

    const cleanedNewTeacherName = String(newTeacherName || "").trim()

    if (!cleanedNewTeacherName) {
      window.alert("Teacher name cannot be blank.")
      return
    }

    updateSemesterTeachers(semesterKey, (currentTeachers) => {
      const maxTeacherId = currentTeachers.reduce(
        (highestId, teacher) => Math.max(highestId, Number(teacher.id) || 0),
        0
      )
  ...
  function promptRenameTeacher(teacherId, currentTeacherName) {
    const nextTeacherName = window.prompt("Rename teacher:", currentTeacherName)

    if (nextTeacherName === null) return

    const cleanedNextTeacherName = String(nextTeacherName || "").trim()

    if (!cleanedNextTeacherName) {
      window.alert("Teacher name cannot be blank.")
      return
    }
  ...
  function renameCourse(teacherId, courseIndex, nextCourseName) {
    updateSemesterTeachers(semesterKey, (currentTeachers) =>
      currentTeachers.map((teacher) => {
        if (teacher.id !== teacherId) return teacher

        const oldCourseName = teacher.courses[courseIndex]
        const cleanedNextCourseName = nextCourseName

        const nextCourses = teacher.courses.map((course, index) =>
          index === courseIndex ? cleanedNextCourseName : course
        )

        const nextAssignments = Object.fromEntries(
          Object.entries(teacher.assignments).map(([blockId, assignment]) => {
            if (assignment.course !== oldCourseName) {
              return [blockId, assignment]
            }
            ...
            return [
              ...
              {
                ...assignment,
                course: cleanedNextCourseName,
                subject: getSubjectFromCourse(cleanedNextCourseName),
              },
            ]
          })
        )
    ...
    setSelectedSwap((currentSelectedSwap) => {
      if (!currentSelectedSwap) return currentSelectedSwap
      const teacher = teachers.find((currentTeacher) => currentTeacher.id === teacherId)
      const oldCourseName = teacher?.courses?.[courseIndex]

      if (currentSelectedSwap.teacherId === teacherId && currentSelectedSwap.course === oldCourseName) {
        return { teacherId, course: nextCourseName }
      }

      return currentSelectedSwap
    })
  }

  function deleteCourse(teacherId, courseIndex, currentCourseName) {
    const isConfirmed = window.confirm(
      `Delete "${currentCourseName}" from this teacher? If it is scheduled in a block, that block will become Prep.`
    )
    ...
    updateSemesterTeachers(semesterKey, (currentTeachers) =>
      currentTeachers.map((teacher) => {
        ...
        const nextCourses = teacher.courses.filter((course, index) => index !== courseIndex)
    ...
    setSelectedSwap((currentSelectedSwap) => {
      if (
        currentSelectedSwap?.teacherId === teacherId &&
        currentSelectedSwap?.course === currentCourseName
      ) {
        return null
      }
  ...
  function promptRenameCourse(teacherId, courseIndex, currentCourseName) {
    const nextCourseName = window.prompt("Rename course:", currentCourseName)

    if (nextCourseName === null) return

    const cleanedNextCourseName = String(nextCourseName || "").trim()

    if (!cleanedNextCourseName) {
      window.alert("Course name cannot be blank.")
      return
    }

    renameCourse(teacherId, courseIndex, cleanedNextCourseName)
  }

  function updateNewCourseDraft(teacherId, value) {
    setNewCourseDrafts((currentDrafts) => ({
      ...currentDrafts,
      [teacherId]: value,
    }))
  }
  ...
  function addCourseToTeacher(teacherId) {
    const nextCourseName = String(newCourseDrafts[teacherId] || "").trim()

    if (!nextCourseName) return
    ...
    updateSemesterTeachers(semesterKey, (currentTeachers) =>
      currentTeachers.map((teacher) => {
        ...
        return {
          ...teacher,
          courses: [...teacher.courses, nextCourseName],
        }
      })
    )
    ...
    <section style={semesterSectionStyle}>
      <div style={semesterHeaderStyle}>
        ...
        <div style={semesterActionsStyle}>
          ...
          <button type="button" onClick={addTeacher} style={secondaryButtonStyle}>
            Add Teacher
          </button>
        ...
        <table style={tableStyle}>
          <thead>
            <tr>
              ...
              <th style={allCoursesHeaderStyle}>All Courses</th>
            ...
            {teachers.map((teacher) => (
              <tr key={teacher.id}>
                <td style={teacherCellStyle}>
                  <div style={teacherNameStyle}>{teacher.name}</div>
                  <button
                    type="button"
                    onClick={() => promptRenameTeacher(teacher.id, teacher.name)}
                    style={renameTeacherButtonStyle}
                  >
                    Rename Teacher
                  </button>
                </td>
                ...
                <td style={wideCellStyle}>
                  <div style={allCoursesGridStyle}>
                    {teacher.courses.map((course, courseIndex) => (
                      <div key={`${teacher.id}-${course}-${courseIndex}`} style={allCourseEditRowStyle}>
                        <div style={allCourseNameStyle}>{course}</div>
                        <div style={courseActionButtonGroupStyle}>
                          <button
                            type="button"
                            onClick={() => promptRenameCourse(teacher.id, courseIndex, course)}
                            style={renameCourseButtonStyle}
                          >
                            Rename
                          </button>
                          <button
                            type="button"
                            onClick={() => deleteCourse(teacher.id, courseIndex, course)}
                  ...
                  <div style={addCourseRowStyle}>
                    <input
                      value={newCourseDrafts[teacher.id] || ""}
                      onChange={(event) => updateNewCourseDraft(teacher.id, event.target.value)}
                      placeholder="Add new course"
                      style={addCourseInputStyle}
                      aria-label={`Add course for ${teacher.name}`}
                    />
                    <button
                      type="button"
                      onClick={() => addCourseToTeacher(teacher.id)}
...
const renameTeacherButtonStyle = {
  border: "1px solid #9ca3af",
  background: "#ffffff",
  color: "#111",
  borderRadius: "999px",
...
const courseActionButtonGroupStyle = {
  display: "flex",
  alignItems: "center",
  gap: "6px",
  flexShrink: 0,
...
const addCourseRowStyle = {
  display: "flex",
  gap: "8px",
  marginTop: "10px",
}

const addCourseInputStyle = {
  border: "1px solid #9ca3af",
  borderRadius: "10px",
  padding: "8px 10px",
  background: "#ffffff",
