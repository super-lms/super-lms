...
async function ensureRubricFrameworkTables() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS rubric_repository (
      id SERIAL PRIMARY KEY,
      title TEXT NOT NULL,
      ...
      parent_rubric_id INTEGER REFERENCES rubric_repository(id) ON DELETE SET NULL,
  ...
  for (const row of rows) {
    ...
    if (userResult.rows.length > 0) {
      ...
      await pool.query(
        ...
        WHERE id = $5
        `,
        [firstName, lastName, parentEmail, studentId, userId]
      );
      ...
      const insertedUserResult = await pool.query(
        `
        INSERT INTO users (first_name, last_name, email, password_hash, role, parent_email, student_id)
        VALUES ($1, $2, $3, 'TEMP_PASSWORD_NEEDS_RESET', $4, $5, $6)
        RETURNING id
        `,
        [firstName, lastName, studentEmail, "student", parentEmail, studentId]
      );
...
async function ensureDemoStudentsForClass(classId) {
  ...
  for (const student of demoStudents) {
    const userResult = await pool.query(
      ...
      `,
      [
        student.name.split(/\s+/).filter(Boolean)[0] || "Student",
        student.name.split(/\s+/).filter(Boolean).slice(1).join(" ") || "User",
        student.email,
        student.parentEmail,
        student.studentId,
      ]
...
app.get("/api/courses", authenticateJWT, requireRole("admin", "teacher", "student", "observer"), async (req, res) => {
  try {
    ...
    if (hasNameColumn) {
      teacherNameSql = "COALESCE(NULLIF(TRIM(CONCAT(u.first_name, ' ', u.last_name)), ''), u.email, 'Unknown Teacher')";
    ...
    if (submissionsResult.rows.length === 0 && observerStudents.source === "grade_11_fallback") {
      submissionsResult = await pool.query(
        ...
        GROUP BY
          ...
          u.first_name,
          u.last_name
        ORDER BY COALESCE(NULLIF(TRIM(CONCAT(u.first_name, ' ', u.last_name)), ''), ms.display_name) ASC, c.title ASC, a.title ASC
        `
      );
    }
...
app.post("/api/assignments/:assignmentId/kdu-scores", authenticateJWT, requireRole("admin", "teacher"), async (req, res) => {
  try {
    ...
    if (!courseId) {
      return res.status(400).json({ error: "Assignment class_id is required before saving KDU scores" });
    }
